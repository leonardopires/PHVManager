...
fim:
    ...
    Public Sub savePhoto()
        ...
        ab = New SqlClient.SqlCommand("insert into imagem ([imagem], [filesize], [mimetype], [datetime], [filename]) values (@pFoto, @pFotoTamanho, @pMimeType, @pDataHora, @pArquivo)", g.cn)
        ...
        Dim a As New SqlClient.SqlDataAdapter("select id_imagem as id_foto from imagem where (datetime = @pDataHora and filename = @pArquivo)", g.cn)
        ...
        If Not System.Convert.ToString(Session.Item("fotoID")) = "" Then
            ...
            a.UpdateCommand = New SqlClient.SqlCommand("update imagem_noticia set id_imagem = null where id_imagem = @pID", g.cn)
            ...
            a.UpdateCommand.CommandText = "update cliente set id_imagem = null where id_imagem = @pID"
            a.UpdateCommand.Parameters.Item("@pID").Value = SqlTypes.SqlGuid.Parse(System.Convert.ToString(Session.Item("fotoID")))
            g.cn.Close()
            g.cn.Open()
            a.UpdateCommand.ExecuteNonQuery()
            g.cn.Close()

            a.UpdateCommand.CommandText = "update produto_imagem set id_imagem = null where id_imagem = @pID"
            a.UpdateCommand.Parameters.Item("@pID").Value = SqlTypes.SqlGuid.Parse(System.Convert.ToString(Session.Item("fotoID")))
            g.cn.Close()
            g.cn.Open()
            a.UpdateCommand.ExecuteNonQuery()
            ...
            a.UpdateCommand.CommandText = "update imagem_institucional set id_imagem = null where id_imagem = @pID"
            a.UpdateCommand.Parameters.Item("@pID").Value = SqlTypes.SqlGuid.Parse(System.Convert.ToString(Session.Item("fotoID")))
            g.cn.Close()
            g.cn.Open()
            a.UpdateCommand.ExecuteNonQuery()
            ...
            a.DeleteCommand = New SqlClient.SqlCommand("delete from imagem where id_imagem = @pID", g.cn)
    ...
    Private Function GetImages(ByVal imageId As SqlTypes.SqlGuid) As SqlClient.SqlDataReader
        ...
        Command = New SqlClient.SqlCommand("Select * From imagem Where id_imagem=@ImageId", g.cn)
