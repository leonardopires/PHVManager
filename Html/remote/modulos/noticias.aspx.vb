	...
	Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
		...
		If g.logged Then
			...
			Using cn As SqlConnection = AfabreHelper.CreateConnection()
				...
				lstAlterna.Items.FindByValue("noticias").Selected = True
...
#Region "Subrotinas de Interface + DB"
	Private Sub btnIr_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnIr.Click
		...
		Using cn As SqlConnection = AfabreHelper.CreateConnection()
			Dim a As New SqlClient.SqlDataAdapter("select * from noticia n inner join imagem_noticia i on n.id_noticia = i.id_noticia where chamada like @pChamada or texto like @pTexto  order by data desc", cn)
	...
	Private Sub btnPublish_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPublish.Click
		...
		If txtMateria.Text.Length > 1 And txtChamada.Text.Length > 1 Then
			...
			If Session.Item("noticiaAction") = "update" Then
				...
				Using cn As SqlConnection = AfabreHelper.CreateConnection()
					Dim d As New SqlClient.SqlCommand("update texto set titulo = @pChamada, texto = @pTexto where id_texto = @pId", cn)
	...
	Private Sub calNews_VisibleMonthChanged(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.MonthChangedEventArgs) Handles calNews.VisibleMonthChanged
		Using cn As SqlConnection = AfabreHelper.CreateConnection()
			Dim a As New SqlClient.SqlDataAdapter("select n.texto as chamada, n.id_texto as id_noticia, n.data, n.texto, i.id_texto, i.id_imagem from texto n inner join area a on a.id_area = n.id_area inner join imagem_texto i on n.id_texto = i.id_texto where a.nomept = @area and month(data) = @pData order by data desc", cn)
	...
	Private Sub lstNoticias_DeleteCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataListCommandEventArgs) Handles lstNoticias.DeleteCommand
		...
		Using cn As SqlConnection = AfabreHelper.CreateConnection()
			Dim a As New SqlClient.SqlDataAdapter("select id_texto as id_noticia from imagem_texto where id_texto = @pId", cn)
...
#Region "Subrotinas de DB"
	Public Sub editNoticia(ByVal id As SqlTypes.SqlGuid)
		Using cn As SqlConnection = AfabreHelper.CreateConnection()
			Dim a As New SqlClient.SqlDataAdapter("select n.titulo as chamada, n.texto, n.data, n.id_texto as id_noticia, i.id_imagem from texto n inner join area a on a.id_area = n.id_area inner join imagem_texto i on n.id_texto = i.id_texto where n.id_texto = @pId and a.nomept = @area", cn)
	...
	Public Sub delNoticia(ByVal id As SqlTypes.SqlGuid)
		...
		Using cn As SqlConnection = AfabreHelper.CreateConnection()
			...
			Dim a As New SqlClient.SqlDataAdapter("select * from imagem_texto where id_texto = @pId", cn)
			a.SelectCommand.Parameters.Add("@pId", id)
			If g.ds.Tables.Contains("delNoticia") Then g.ds.Tables("delNoticia").Clear()
			a.Fill(g.ds, "delNoticia")


			Dim d As New SqlClient.SqlCommand("delete from imagem_texto where id_texto = @pId", cn)
	...
	Public Sub insertNoticia()
		...
		Using cn As SqlConnection = AfabreHelper.CreateConnection()
			Dim a As New SqlClient.SqlDataAdapter("select * from noticia", cn)
			Dim data As New DateTime

			data = Date.Now

			Dim b As New SqlClient.SqlDataAdapter("select * from area where nomept=@area", cn)
			b.SelectCommand.Parameters.Add("@area", "Notícias")
			...
			a.InsertCommand = New SqlClient.SqlCommand("insert into texto (titulo, texto, data, id_area) values (@pChamada, @pTexto, @pData, @area)", cn)
